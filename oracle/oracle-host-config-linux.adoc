...
（a）DNFSが有効で、（b）ソースNFS共有が単一のサーバに複数回マウントされ、（c）ネストされたNFSマウントがある場合、 `nosharecache`と呼ばれる追加のマウントオプションが必要です。この構成は、主にSAPアプリケーションをサポートする環境で見られます。たとえば、NetAppシステム上の単一のNFS共有に、 `/vol/oracle/base`にあるディレクトリと `/vol/oracle/home`にある2つ目のディレクトリがあるとします。 `/vol/oracle/base`が `/oracle`にマウントされ、 `/vol/oracle/home`が `/oracle/home`にマウントされている場合、同じソースから発生するネストされたNFSマウントになります。

OSは、 `/oracle`と `/oracle/home`が同じソース ファイル システムである同じNFS共有に存在することを検出できます。その後、OSはデータへのアクセスに同じデバイス ハンドルを使用します。これにより、OSキャッシングやその他の特定の処理の利用が向上しますが、DNFSと干渉します。DNFSが `/oracle/home`上の `spfile`などのファイルにアクセスする必要がある場合、誤ってデータへの間違ったパスを使用しようとする可能性があります。その結果、I/O処理が失敗します。これらの構成では、そのホスト上の別のNFSファイル システムとソースNFS共有を共有するすべてのNFSファイル システムに、 `nosharecache`マウント オプションを追加します。これにより、Linux OSは強制的にそのファイル システムに独立したデバイス ハンドルを割り当てます。
