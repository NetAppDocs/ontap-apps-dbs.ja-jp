...
最も重要な要件は、データファイルを1つまたは複数の専用コンテナに分離することです。これは、すべてのデータファイルを単一のLUN/ネームスペースに配置する、単一のAFF NFSボリューム内に配置する、またはマルチボリュームやマルチLUN/ネームスペースのONTAP整合性グループに配置するなど、いくつかの方法で実行できます。重要なのは、データファイルが他のファイルタイプによって汚染されていないことです。その理由は、データファイルのレプリケーションが、アーカイブログなどの他のデータタイプのレプリケーションとは完全に独立していることを保証するためです。ファイル レイアウトに関する追加情報、およびストレージ レイアウトがSnapshotに対応していることを保証するための重要な詳細については、link:oracle-dp-online-backup.html#data-layout["データレイアウト"]を参照してください。
