...
VAAIはストレージにオフロードされるさまざまな操作を提供していますが、VASAプロバイダーによって対処されるいくつかのギャップが存在します。SAN VAAIは、VMwareが管理するスナップショットをストレージシステムにオフロードすることができません。NFS VAAIはVMが管理するスナップショットをオフロードできますが、ストレージネイティブのスナップショットを持つVMには制限があります。vVolsは仮想マシンディスクに個別のLUN、名前空間、またはファイルを使用するため、ONTAPはファイルやLUNを迅速かつ効率的にクローンして、デルタファイルを必要としないVM単位のスナップショットを作成できます。NFS VAAIは、ホット（電源オン）Storage vMotion移行のクローン操作のオフロードもサポートしていません。VAAIを従来のNFSデータストアで使用する場合、移行処理のオフロードを可能にするためにVMの電源をオフにする必要があります。ONTAPツールのVASAプロバイダーを使用すると、ホット移行とコールド移行のためのストレージ効率に優れたクローンをほぼ瞬時に作成でき、vVolsのボリューム間移行のためのコピーもほぼ瞬時にサポートします。これらの大きなストレージ効率のメリットにより、 https://www.netapp.com/pdf.html?item=/media/79014-ng-937-Efficiency-Guarantee-Customer-Flyer.pdf["容量削減保証"]プログラムのもとでvVolsワークロードを最大限に活用できる可能性があります。同様に、VAAIを使用したクロスボリュームクローンが要件を満たさない場合でも、vVolsによるコピーエクスペリエンスの改善により、ビジネス上の課題を解決できる可能性があります。
