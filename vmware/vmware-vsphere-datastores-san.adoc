...
vSphereには、ストレージ デバイスへの複数のパスに対する組み込みサポートが含まれています。vSphereは、サポートされているストレージ システムのストレージ デバイスのタイプを検出し、使用されているプロトコルに関係なく、また ASA、AFF、FAS、またはソフトウェアで定義される ONTAP を使用しているかどうかに関係なく、使用中のストレージ システムの機能をサポートするようにマルチパス スタックを自動的に設定できます。
...
NOTE: ESXiでサポートされている最新の制限事項については、link:https://configmax.broadcom.com/guest?vmwareproduct=vSphere&release=vSphere%208.0&categories=2-0["VMware Configuration Maximumsツール"^]を参照してください。
...
* iSCSI または NVMe/TCP ネットワークの場合、複数の仮想スイッチが存在する場合は、NIC チーミングを使用して、異なるルーティングされていないネットワーク サブネット上の複数の VMkernel ネットワーク インターフェイスを使用してください。複数の物理 NIC を複数の物理スイッチに接続することで、HA を実現し、スループットを向上させることもできます。ONTAP で、フェイルオーバー用に、2 つ以上のスイッチに接続された 2 つ以上のリンクを持つシングルモード インターフェイス グループを構成するか、または LACP またはその他のリンク アグリゲーション技術をマルチモード インターフェイス グループで使用して、HA とリンク アグリゲーションの利点を提供します。NVMe/TCP は VMkernel ポートごと、および物理インターフェイスごとに有効にする必要があり、NVMe/TCP VMkernel ポートが NVMe/TCP が有効になっている物理 NIC にアクセスできるように設定する必要があることに注意してください。次の図は、IP ベースの SAN プロトコルによるマルチパス接続を示しています。image:vsphere_ontap_image2.png["マルチハスセツソク"]
...
* NVMe/FC と vSphere 7.0 の使用方法の詳細については、こちら https://docs.netapp.com/us-en/ontap-sanhost/nvme_esxi_7.html["ONTAP NVMe/FC Host Configuration Guide"^]および https://www.netapp.com/pdf.html?item=/media/10681-tr4684.pdf["TR-4684"^]を参照してください。次の図は、vSphere ホストから ONTAP LUN へのマルチパス接続を示しています。
