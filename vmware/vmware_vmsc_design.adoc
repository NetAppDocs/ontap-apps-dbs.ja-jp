...
ONTAP ツール vCenter プラグインは、vMSC の SnapMirror アクティブ同期を設定するためのシンプルで直感的な方法を提供します。ONTAP ツール vCenter プラグインを使用して、2 つの ONTAP クラスター間の SnapMirror アクティブ同期関係を作成および管理できます。このプラグインは、これらの関係を効率的に確立および管理するための使いやすいインターフェイスを提供します。ONTAP ツール vCenter プラグインの詳細については、 https://docs.netapp.com/us-en/ontap-tools-vmware-vsphere-10/index.html["VMware vSphere 用の ONTAP ツール"]を参照するか、すぐに https://docs.netapp.com/us-en/ontap-tools-vmware-vsphere-10/protect/granular-datastore-protection-concept.html["ONTAP tools における詳細なデータストア保護の適格性基準"]に進んでください。
