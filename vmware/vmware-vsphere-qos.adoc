...
NOTE: vVolsでは、ONTAP の VASA Provider として機能する ONTAP tools for VMware vSphere を使用する必要があります。vVols のベストプラクティスについては、link:vmware-vvols-overview.html["VMware vSphere Virtual Volume（VVOL）とONTAP"]を参照してください。
