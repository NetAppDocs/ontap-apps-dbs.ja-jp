...
ONTAP 9.12.1以降、Snapshotは読み取り専用であるだけでなく、偶発的または意図的な削除から保護することもできます。この機能は改ざん防止Snapshotと呼ばれています。Snapshotポリシーを通じて、保持期間を設定し、適用することができます。作成されたSnapshotは、有効期限が切れるまで削除できません。管理者やサポート センターによる上書き機能はありません。
